...
def train(model, train_loader, optimizer, epoch):
    ...
    for idx, (user_id, in_data, gt_data) in enumerate(train_loader):
        ...
        pred_time = torch.sigmoid(pred_time) * 2 - 1

        gt_loc = gt_data[:, 0:2]

        last_checkin_time = in_data[:, -1].data
        gt_checkin_time = gt_data[:, -1].data
        gt_time = gt_checkin_time - last_checkin_time
...
def test(model, val_loader, epoch):
    ...
    for idx, (user_id, in_data, gt_data) in enumerate(val_loader):
        ...
        # create gt_time
        last_checkin_time = in_data[:, -1:]
        gt_checkin_time = gt_data[:, 2:]

        gt_time = gt_checkin_time

        pred_time = pred_time + last_checkin_time
        pred_time[pred_time < 0] = 0
        pred_time[pred_time > 1] = 1
