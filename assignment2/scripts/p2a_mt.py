...
def main():
   ...
   # create dataloader
    ...
    copy_model_variables(model, ema_model)
    detach_model(ema_model)

    logger.info('ema_model: \n  {0}\n'.format(args['model']))
    logger.info(models.model_str(ema_model))
    ...
    if args['test']:
        ...
        test(model, ema_model, test_loader, 0)
        return
    ...
    for epoch in range(start_epoch, args['epochs']):
        ...
        train(model, ema_model, train_loader, optimizer, epoch)

        if epoch % args['val_freq'] == 0:
            logger.info('--- start to test epoch: {0} ---'.format(epoch))
            test(model, ema_model, test_loader, epoch)
...
def train(model, ema_model, train_loader, optimizer, epoch):
    global global_step
    ...
    for idx, (user_id, in_data, gt_data) in enumerate(train_loader):
        ...
        # pred_time = torch.sigmoid(pred_time)

        ema_pred_loc, ema_pred_time = ema_model.forward(in_data)
        # ema_pred_loc = torch.sigmoid(ema_pred_loc)
        # ema_pred_time = torch.sigmoid(ema_pred_time)
        ...
        ema_loc_loss = 0 * args['loc_scale'] * mse_loss(pred_loc, ema_pred_loc)
        ema_time_loss = 0 * args['time_scale'] * mse_loss(pred_time, ema_pred_time)

        meters.update('loc_loss', loc_loss.data)
        meters.update('time_loss', time_loss.data)

        loss = loc_loss + time_loss + ema_loc_loss + ema_time_loss
        loss.backward()
        optimizer.step()

        update_ema_variables(model, ema_model, args['ema_coeff'], global_step)
        global_step += 1
...
def test(model, ema_model, val_loader, epoch):
    """
    ...
    for idx, (user_id, in_data, gt_data) in enumerate(val_loader):
        ...
        ema_loc_l1 = getSAD(ema_pred_loc, gt_loc)
        ema_time_l1 = getSAD(ema_pred_time, gt_time)
        meters.update('ema_loc_l1', ema_loc_l1)
        meters.update('ema_time_l1', ema_time_l1)
        ...
        if idx % args['log_freq'] == 0:
            logger.info('step: [{0}][{1}/{2}]\t'
                        ...
                        'ema_loc_l1: {meters[ema_loc_l1]:.4f}\t'
                        'ema_time_l1: {meters[ema_time_l1]:.4f}\t'
